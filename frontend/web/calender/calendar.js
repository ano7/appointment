...
function changedateadmin(btn, layout) {
   if (btn == "prevyr") {
   ...
   // console.log("yearNum_" + layout + ", monthNum_" + layout);
   ...
   // Create calendar
   eval("createCalendarAdmin(layout, firstDay, numbDays, monthNum_" + layout + ", yearNum_" + layout + ");");

   return;
}

// Create calendar
function createCalendarAdmin(layout, firstDay, numbDays, monthNum, yearNum) {
   calendarString = '';
   daycounter = 0;
   ...
   calendarString += '<tr>';
   if (layout == 'full') {
      calendarString += '<td class=\"calendar-btn\"><span onClick=\"changedateadmin(\'prevyr\', \'full\')\">« <span class="btn-change-date">' + prev_year + '<\/span><\/span><\/td>';
      calendarString += '<td class=\"calendar-btn\"><span onClick=\"changedateadmin(\'prevmo\', \'full\')\">« <span class="btn-change-date">' + prev_month + '<\/span><\/span><\/td>';
      calendarString += '<td class=\"calendar-title\" colspan=\"3\"><span><i class=\"fa fa-calendar-o\"><\/i>' + wordMonth[monthNum - 1] + '  ' + yearNum + '<\/span><\/td>';
      calendarString += '<td class=\"calendar-btn\"><span onClick=\"changedateadmin(\'nextmo\', \'full\')\"><span class="btn-change-date">' + next_month + '<\/span> »<\/span><\/td>';
      calendarString += '<td class=\"calendar-btn\"><span onClick=\"changedateadmin(\'nextyr\', \'full\')\"><span class="btn-change-date">' + next_year + '<\/span> »<\/span><\/td>';
   } else {
      calendarString += '<td class=\"calendar-btn\"><span onClick=\"changedateadmin(\'prevyr\', \'compact\')\">«<\/span><\/td>';
      calendarString += '<td class=\"calendar-btn\"><span onClick=\"changedateadmin(\'prevmo\', \'compact\')\">«<\/span><\/td>';
      calendarString += '<td class=\"calendar-title\" colspan=\"3\"><span>' + wordMonth[monthNum - 1] + '  ' + yearNum + '<\/span><\/td>';
      calendarString += '<td class=\"calendar-btn\"><span onClick=\"changedateadmin(\'nextmo\', \'compact\')\">»<\/span><\/td>';
      calendarString += '<td class=\"calendar-btn\"><span onClick=\"changedateadmin(\'nextyr\', \'compact\')\">»<\/span><\/td>';
   }
   calendarString += '<\/tr>';
   ...
   for (var i = 1; i <= 6; i++) {
      var k = (i - 1) * 7 + 1;
      // console.log(k + ' ' + firstDay + numbDays);
         ...
         for (var x = 1; x <= 7; x++) {
            ...
            if ((daycounter > numbDays) || (daycounter < 1)) {
               ...
               if ((todaysDate == daycounter) && (todaysMonth == monthNum)) {
                  calendarString += '<td style="cursor: pointer;" onclick=\"loadScheduleAdmin(\'' + yearNum + '-' + monthNum + '-' + daycounter + '\')\" class=\"calendar-day-normal calendar-day-today\">';
               } else {
                  calendarString += '<td style="cursor: pointer;" onclick=\"loadScheduleAdmin(\'' + yearNum + '-' + monthNum + '-' + daycounter + '\')\" class=\"calendar-day-normal\">';
               }
      ...
      if (k < (firstDay + numbDays)) {
         ...
         for (var x = 1; x <= 7; x++) {
            ...
            if ((daycounter > numbDays) || (daycounter < 1)) {
               ...
               if ((todaysDate == daycounter) && (todaysMonth == monthNum)) {
                  calendarString += '<td style="cursor: pointer;" onclick=\"loadSchedule(\'' + yearNum + '-' + monthNum + '-' + daycounter + '\')\" class=\"calendar-day-normal calendar-day-today\">';
               } else {
                  calendarString += '<td style="cursor: pointer;" onclick=\"loadSchedule(\'' + yearNum + '-' + monthNum + '-' + daycounter + '\')\" class=\"calendar-day-normal\">';
               }
               if (checkEvents(daycounter, monthNum, yearNum)) {
                  if (layout == 'full') {
                     calendarString += '<div class=\"calendar-day-event\">';
...
function getOrderNumber(id, day, month, year) {
   ...
   for (var i = 0; i < tiva_events.length; i++) {
      ...
      if ((start_date.getTime() <= date_check.getTime()) && (date_check.getTime() <= end_date.getTime())) {
         ...
         var event = {
            id: tiva_events[i].id,
            name: tiva_events[i].name,
            day: tiva_events[i].day,
            month: tiva_events[i].month,
            year: tiva_events[i].year,
            first_day: first_day
         };
         events.push(event);
      }
...
// Get instructor detail for booking
function loadScheduleAdmin(Appointment_Date) {
	var dataString = {
	  'Appointment_Date': Appointment_Date
	};
	$.ajax({
		type: 'post',
		url: BASE_URL + "/admin/order/view",
		data: dataString,
		cache: false, 
...
// Get instructor detail for booking
function loadSchedule(Appointment_Date) {
	var url = BASE_URL + "/services/our-service/time-slot";
	if (Appointment_Date == null){
		var Appointment_Date = $('#Appointment_Date').val();
		var url = BASE_URL + "/admin/order/time-slot";
	}
	var dataString = {
	...
	$.ajax({
		...
		cache: false, 
		success: function(data) {
			$("#instructor-timeslot").html(data);
			$('html, body').animate({
				scrollTop: $("#instructor-timeslot").offset().top
			}, 500);
		}
	});
}
jQuery(document).ready(function () {
   ...
   // createCalendar('full', 1, d.getDate(), d.getMonth()+1, d.getFullYear());
